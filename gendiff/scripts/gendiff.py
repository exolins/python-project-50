...
def main():
    ...
    #     print(file1[col])
    #     result.append(Diff(key, value, '-'))
    # for key, value in file1.items():
    #     if key in map(lambda r: r.key(), result):
            
            
            
    # for col in file2:
    #     if col in file1:
    #         if file1[col] == file2[col]:
    #             result.pop(('-', col))
    #             result[' ', col] = file1[col]
    #         else:
    #             result['+', col] = file2[col]
    # sorted(result, key=lamda l: l.keys().[1])
    for k in file1:
        if k in file2:
            if file2[k] == file1[k]:
